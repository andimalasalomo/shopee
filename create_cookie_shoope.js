...
async function loginShopee(email, password, id_akun, id_worker, lang) {
        ...
        try {
            ...
            if (state === "hide") {
                ...
                await session.send("Browser.setWindowBounds", {
                    ...
                    bounds: {
                        windowState: "normal",
                        left: 0, // WAJIB ADA: Narik dari -2000 ke 0 agar terlihat lagi
                        top: 0,
                        width: 720,
                        height: 720,
                    },
    ...
    try {
                    ...
                    const secondPopupPromise = new Promise((resolve, reject) => {
                        ...
                        const listener = async (t) => {
                            ...
                            try {
                                ...
                                // Tambahkan script untuk menghilangkan jejak webdriver
        ...
        try {
            ...
            while (Date.now() - startSetuju < 3000) { // Dikurangi dari 8000ms
                // Timeout 8 detik
        ...
        if (captchaElement) {
            ...
            while (true) {
                try {
                    ...
                    if (!sliderNode || !sliderNode.nodeId) {
                        ...
                        if (!stillExist) {
                            ...
                            if (lastSrc) {
                                ...
                                if (isAutoResolved) {
                                    await pool.query(
                                        "UPDATE captcha_shopee SET total_otomatis = total_otomatis + 1 WHERE image_hash = ?",
                                        [imgHash],
                                    );
                                    console.log("✅ Data total_otomatis ditambah 1 di database.");
                                } else {
                                    // await pool.query(
                                    //     "UPDATE captcha_shopee SET data_resolve = ?, puzzleX = ?, puzzleY = ?, sliderX = ?, status = 1 WHERE status = 0 AND image_hash = ?",
                                    //     [resolveData, numPuzzleX, numPuzzleY, numSliderX, imgHash],
                                    // );
                    ...
                    if (currentSrc) {
                        if (currentSrc) { // Pengganti kurung kurawal if(srcIdx !== -1)
                            ...
                            if (currentSrc !== lastSrc) {
                                ...
                                let [cekCaptcha] = await pool.query(
                                    "SELECT id, data_resolve, status, sliderX FROM captcha_shopee WHERE image_hash = ?",
                                    [imgHash],
                                );
                                ...
                                if (cekCaptcha.length > 0) {
                                    ...
                                    console.log("Captcha Lama ditemukan");

                                    if (
                                        cekCaptcha[0].status === 1 &&
                                        ...
                                        await pool.query(
                                            "UPDATE captcha_shopee SET total_show_setelah_resolve = total_show_setelah_resolve + 1 WHERE id = ?",
                                            [cekCaptcha[0].id],
                                        );

                                        const resolveData = cekCaptcha[0].data_resolve;
                                        const matchX = resolveData.match(
                                            /#modal[\s\S]*?translateX\(([-.\d]+)px\)/,
                                        );

                                        let sliderXVal = cekCaptcha[0].sliderX;
                                        if (sliderXVal != null && parseFloat(sliderXVal) !== 0) {
                                            let distance = parseFloat(sliderXVal);
                                            ...
                                            if (sliderNode && sliderNode.nodeId) {
                                                ...
                                                if (model) {
                                                    ...
                                                    isAutoResolved = true;
                                                    console.log(
                                                        `✅ Slider ${cekCaptcha[0].id} berhasil ditarik otomatis sejauh ${distance}px berdasarkan database.`,
                                                    );
                                                }
                                            }
                                        }
                    ...
                    if (nodeP && nodeP.nodeId) {
                        ...
                        if (currentX !== lastX) {
                            if (
                                ...
                                const matchY = pY.match(/translateY\(([-\d.]+)px\)/);

                                if (matchX) numPuzzleX = parseFloat(matchX[1]);
                                if (matchY) numPuzzleY = parseFloat(matchY[1]);

                                numSliderX = numPuzzleX; // Asumsikan pergeseran slider sama dengan puzzle
